...
# Metaformers — self-recursive v2.0 (lean)
...
TOPIC_DEFAULT="You are three AI models working collaboratively. Your goal is to find the right prompts for a self-improving LLMs stack that can build with python and bash."
: "${AUTO_CHAIN:=0}"
...
log_line(){ print -- "[$(ts)] $*" >> "$1"; }
log_both(){ local target="$1"; shift; local line="[$(ts)] $*"; if [[ "$target" == "$master" ]]; then print -- "$line" >> "$master"; else print -- "$line" | tee -a "$target" >> "$master"; fi }

[[ -x "$ollama_bin" ]] || { echo "[$(ts)] [fatal] ollama not found at: $ollama_bin" | tee -a "$master" >&2; exit 3; }

ollama_run(){ local model="$1"; shift; local prompt="$*"; ( print -r -- "$prompt" | "$ollama_bin" run "$model" ) & local pid=$!; local limit=$((5*60)); while kill -0 $pid 2>/dev/null; do sleep 1; ((limit--)); if (( limit<=0 )); then kill -TERM $pid 2>/dev/null || true; sleep 1; kill -KILL $pid 2>/dev/null || true; echo "[timeout] ollama run exceeded 5m for model=$model" >&2; return 124; fi; done; wait $pid; }
...
if [[ -n "${TOPIC_OVERRIDE:-}" ]]; then
  ...
  if [[ -n "$last_run" && -f "$last_run/seed_prompt.txt" ]]; then current_prompt="$(cat "$last_run/seed_prompt.txt")"; PARENT_ID="$(basename "$last_run")"; fi
fi
...
q_log="$LOG_DIR/questioner_$RUN_ID.log"; c_log="$LOG_DIR/creator_$RUN_ID.log"; m_log="$LOG_DIR/mediator_$RUN_ID.log"; master="$LOG_DIR/master_$RUN_ID.log"
: > "$q_log"; : > "$c_log"; : > "$m_log"; : > "$master"
...
generate_index(){
  local INDEX="$ROOT/runs_index.md"
  {
    ...
    echo
    echo "| Run ID | Parent | Started (UTC) | Ended (UTC) | Models | Principles | Logs | Seed |"
    echo "|---|---|---|---|---|---|---|---|"
    ls -1dt "$RUNS_DIR"/* 2>/dev/null | while read -r dir; do
      [[ -d "$dir" ]] || continue
      local rid started ended q c m models p_link seed_link logs_link parent
      rid="${dir##*/}"
      ...
      echo "| $rid | ${parent:--} | ${started:--} | ${ended:--} | $models | $p_link | [logs]($logs_link) | $seed_link |"
    done
    echo
