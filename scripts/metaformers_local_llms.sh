...
# ===========================
# Metaformers — self-recursive v3.1 (local-only, disk-budgeted)
# ===========================
# Local, hermetic, Automator-safe. No Homebrew deps. Uses BSD awk/sed.
# v3.1 adds: explicit 20GB disk cap tracking, per-iteration delta checks,
# Mediator enforcement of executable steps, and Creator prompt context
# with remaining disk budget.
...
# === Disk budget (total cap = 20GB; per-iteration new artifacts <2GB) ===
DISK_CAP_BYTES=$((20 * 1024 * 1024 * 1024))   # 20 GiB
DELTA_CAP_BYTES=$((2 * 1024 * 1024 * 1024))   # 2 GiB

# === Seed behavior ===
TOPIC_DEFAULT="You are three AIs (Questioner=Llama2, Creator=gpt-oss, Mediator=Dolphin) designing a SELF-IMPROVING local LLM stack using ONLY:
- open-source models runnable via Ollama,
- a Mac mini with M4 Pro,
- Python 3.x + Postgres (psycopg),
- NO cloud, NO external APIs, air-gapped assumptions.

Goals per iteration:
1) Propose a concrete change that can be implemented locally (training, fine-tuning, prompting, routing, adapters, caching, calibration, eval).
2) Specify exact on-disk artifacts: code files to write/update, tables to create in Postgres, and CLI commands to run.
3) Keep compute budget: ≤+10% tokens/sec slowdown vs baseline; disk <2 GB new artifacts per iter.
4) Include a micro-eval we can run offline on a small fixture set (≤200 prompts): metrics = ECE, exact-match or Rouge-L (as relevant), latency.
5) Disk budget enforcement: each iteration MUST output 'ls -lh' for touched paths AND 'du -sh ./local-meta', and subtract from the 20 GB total cap.

Mediator (Dolphin) — enforcement rule:
- Reject any Creator output that is not immediately executable locally. Force exact file paths, Postgres DDL, and single-line shell commands. If vague, rewrite into an executable plan."
...
LOCAL_META_DIR="$ROOT/local-meta"
mkdir -p "$LOG_DIR" "$META_DIR" "$CAND_DIR" "$LOCAL_META_DIR"
...
bytes_to_gb(){ /usr/bin/awk -v b="$1" 'BEGIN{printf("%.2f", b/1073741824)}'; }
du_bytes(){
  # Returns bytes used by a path (0 if missing)
  local path="$1"
  if [[ -e "$path" ]]; then
    # du -sk returns KiB on macOS; multiply by 1024
    local kib=$(/usr/bin/du -sk "$path" 2>/dev/null | /usr/bin/awk '{print $1+0}')
    /usr/bin/awk -v k="$kib" 'BEGIN{print k*1024}'
  else
    echo 0
  fi
}
...
for (( i = 1; i <= iterations; i++ )); do
  ...
  pre_bytes="$(du_bytes "$LOCAL_META_DIR")"
  pre_gb="$(bytes_to_gb "$pre_bytes")"
  rem_bytes=$(( DISK_CAP_BYTES - pre_bytes ))
  (( rem_bytes < 0 )) && rem_bytes=0
  rem_gb="$(bytes_to_gb "$rem_bytes")"
  log_both "$master" "[disk] pre=$(bytes_to_gb $pre_bytes) GB, remaining=$(bytes_to_gb $rem_bytes) GB (cap=20.00 GB)"
  ...
  # Creator prompt with hard constraints + current disk budget context
  c_prompt="The Questioner asks:
$q_resp

Context (hard constraints):
- Local-only; no cloud; Mac mini M4 Pro; Ollama + Python + Postgres.
- Per-iteration disk delta <2 GB; total cap 20 GB.
- Current ./local-meta usage: $(printf "%.2f" "$pre_gb") GB; remaining budget: $(printf "%.2f" "$rem_gb") GB.

You are the Creator. Using the above local-only constraints and REQUIRED format from the seed, respond in EXACTLY this format:

## Conceptual Insight
(2–4 sentences tying the proposal to self-improvement under local-only constraints)

## Practical Mechanism
1. Files to add/update (paths under ./local-meta): <path>: <brief purpose>
2. Postgres changes (DDL): <CREATE TABLE…> or 'none'
3. Pipeline steps (shell): numbered single-line commands we can run locally
4. Safety & budget checks: the exact asserts we’ll run (compute, disk, guardrails)

## Why This Matters
- Bullet on reliability under tight compute
- Bullet on measurable self-improvement signal
- Bullet on maintainability on a single Mac mini

Report:
- Baseline vs After: {tokens/sec, ECE, task metric} with how to measure locally
- Guardrails: ≤1 re-decode, no retrieval, +10% max slowdown, disk cap 20 GB
- MUST include: final 'ls -lh' for touched paths AND 'du -sh ./local-meta' to report actual disk usage."
  ...
  # Mediator every N — explicitly enforce executability
  if (( i % mediator_every == 0 )); then
    m_prompt="You are the Mediator AI (Dolphin). ENFORCE EXECUTABILITY.
Reject or rewrite any vague parts of the Creator plan into exact, immediately runnable local steps.
Ensure: concrete file paths under ./local-meta, explicit Postgres DDL (or 'none'), and numbered single-line shell commands.
Confirm that disk-budget reporting includes: 'ls -lh' for touched paths AND 'du -sh ./local-meta'.

Context (Creator output):
$c_resp

Return ONE concise meta-question (≤80 words) ending with a '?', OR a minimally-edited executable version if needed."
  ...
  # Snapshot disk after and check deltas
  post_bytes="$(du_bytes "$LOCAL_META_DIR")"
  post_gb="$(bytes_to_gb "$post_bytes")"
  delta_bytes=$(( post_bytes - pre_bytes ))
  (( delta_bytes < 0 )) && delta_bytes=0
  delta_gb="$(bytes_to_gb "$delta_bytes")"

  log_both "$master" "[disk] post=$(bytes_to_gb $post_bytes) GB, delta=$(bytes_to_gb $delta_bytes) GB"

  # Guardrails: per-iteration delta and total cap
  if (( delta_bytes > DELTA_CAP_BYTES )); then
    log_both "$master" "[GUARDRAIL VIOLATION] Iteration $i created >2GB artifacts (delta=${delta_gb} GB)."
  fi
  if (( post_bytes > DISK_CAP_BYTES )); then
    log_both "$master" "[GUARDRAIL VIOLATION] Total ./local-meta usage exceeds 20GB (used=${post_gb} GB)."
  fi
done
...
if [[ -s "$SEED.tmp" ]]; then
  {
    echo "You are three AI models working collaboratively to improve a LOCAL-ONLY LLM stack."
