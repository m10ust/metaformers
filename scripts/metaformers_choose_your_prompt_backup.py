...
TURNS      = int(os.environ.get("TURNS", "12"))
...
def make_questioner_prompt(seed: str, last_creator_take: Optional[str]) -> str:
    """Prompt for Questioner to refine/advance the discussion topic."""
    context = f"\nPrevious creator take:\n{last_creator_take}\n" if last_creator_take else ""
    return (
        "You are the Questioner.\n"
        "Your job is to sharpen the topic with one clear, short question that advances the discussion.\n"
        "Do not add commentary; output only the single best question.\n"
        f"Seed topic:\n{seed}\n"
        f"{context}".strip()
    )
...
def main() -> None:
    ...
    for turn in range(1, TURNS + 1):
        ...
        # Questioner sharpens the topic
        q_prompt = make_questioner_prompt(current_topic, last_creator)
        q_text = run_ollama(QUESTIONER, q_prompt, q_log)

        # If questioner outputs nothing usable, keep previous topic
        next_topic = sanitize_block(q_text).strip('"').strip("'")
        if next_topic:
        ...
        if turn % MEDIATOR_EVERY == 0 and creator_text:
            ...
            mediator_q = run_ollama(MEDIATOR, m_prompt, m_log).strip()
            if mediator_q:
                # Use mediator's question as the next topic seed
                current_topic = mediator_q.strip().strip('"').strip("'")
