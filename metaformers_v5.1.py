#!/usr/bin/env python3
"""
Metaformers_v5.2 — Three-LLM Interactive Loop (Local Ollama) + Turn-1 Literal Mode

- NEW: Literal mode toggle for Turn 1:
  * CLI flag: --literal  (forces pass-through)
  * Interactive: "Should Questioner refine your seed...?" (Y/n)
- If literal is chosen, Turn 1 skips the Questioner LLM call and uses the seed as-is.
- Fix: Terminal color legend 'Error/Errors' naming mismatch.
...
CLI
  --seed "..."  --turns 8  --interval 2  --literal \
...
try:
    ...
    ROLE_COLORS = {
        ...
        "Info":       Fore.MAGENTA if COLOR_ENABLED else "",
    }
    ...
    ROLE_COLORS = {
        ...
        "Info":       MAG   if COLOR_ENABLED else "",
    }
...
def choose_model(role: str, available: List[str], default_name: str) -> str:
    ...
    if not available:
        print_color("Error", "No Ollama models detected. Pull models with `ollama pull <model>`.")
        sys.exit(1)
...
def ollama_chat(model: str, messages: List[Dict[str, str]],
                temperature: float=0.7, top_p: float=0.95, timeout: int=360) -> str:
    payload = {
        "model": model,
        "messages": messages,
        "stream": False,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Metaformers_v5.2 — local multi-agent loop (Ollama) with Markdown logging + index.md")
    ...
    p.add_argument("--literal", action="store_true",
                   help="Turn-1 literal mode: skip Questioner and pass seed directly to Creator.")
    return p.parse_args()
...
def main():
    ...
    print("Metaformers_v5.2 — Local Multi-Agent Orchestrator (Ollama)")
    ...
    # Interactive literal toggle (unless already forced by --literal)
    if args.literal:
        literal_turn1 = True
    else:
        refine = yes_no("Should Questioner refine your seed into a concise question?", default=True)
        literal_turn1 = not refine
    ...
    print(f"  Questioner mode: {'literal (pass-through)' if literal_turn1 else 'refine (rephrase)'}")
    print(f"  Turns: {turns}")
    ...
    logger = RunLogger(
        ...
        config={
            ...
            "MediatorQ": m_model,
            "Scriber": s_model,
            "questioner_mode": "literal" if literal_turn1 else "refine",
        },
    )
    banner_legend()
    ...
    for turn in range(1, turns+1):
        ...
        # ---------------- Questioner ----------------
        try:
            if turn == 1 and literal_turn1:
                # Literal pass-through of the seed; no model call
                q_out = seed_prompt.strip()
                print_color("Questioner", f"Q{turn} (pass-through): {q_out}\n")
                ...
                q_out = ollama_chat(q_model, q_msgs).strip()
                if not q_out:
            ...
            # Fallback: if Q failed, continue with seed (turn 1) or previous question
            q_out = seed_prompt if (turn == 1) else (prev_creator or seed_prompt)

        # ---------------- Creator ----------------
        try:
            c_msgs = build_creator_prompt(q_out)
            c_out = ollama_chat(c_model, c_msgs).strip()
            if not c_out:
        ...
        if mediator_triggered:
            try:
                ...
                mediator_out = ollama_chat(m_model, m_msgs,
                                           temperature=max(0.2, 0.7/2), top_p=0.95).strip()
                if not mediator_out:
